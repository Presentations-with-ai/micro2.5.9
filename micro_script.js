...
const APP_NEWS_ITEMS = window.APP_NEWS_ITEMS || [
  {
    ...
    items: [
      ...
      'Раздел «Изучить тесты» снова открыт для всех пользователей.',
...
function getDailyLimitResultNoteHtml(snapshot) {
  ...
  if (normalized.isPremium) return '<div class="result-note">♾️ Лимит тестов: без ограничений</div>';
  return `<div class="result-note">🧮 Осталось тестов за 24 часа: <b>${escapeHtml(String(normalized.remaining))}</b> из ${escapeHtml(String(normalized.limit))}</div>`;
}
...
function ensureStudyPremiumRequiredModal() {
  ...
  modal.innerHTML = `
    <div class="agreement-panel subscription-panel">
      ...
      <p class="agreement-lead">Раздел с просмотром вопросов и правильных ответов доступен всем пользователям. Премиум по-прежнему снимает лимиты тестов и открывает расширенные режимы.</p>
...
function ensurePremiumActivationModal() {
  ...
  modal.innerHTML = `
    <div class="agreement-panel subscription-panel">
      ...
      <div class="subscription-limit-info">Премиум-доступ снимает ограничения временного пользователя: 3 теста за 24 часа, максимум 50 вопросов в тесте и максимум 60 секунд на вопрос.</div>
...
function ensureSubscriptionPurchaseModal() {
  ...
  modal.innerHTML = `
    <div class="agreement-panel subscription-panel">
      ...
      <div class="subscription-limit-info">Для временного пользователя действует ограничение: 3 теста за 24 часа, до 50 вопросов в одном тесте и до 60 секунд на вопрос. С премиум-подпиской эти ограничения снимаются.</div>
